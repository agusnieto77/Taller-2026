...
def test_initial_migration_and_model_metadata_enforce_unique_usernames(tmp_path: Path) -> None:
...
def test_seed_demo_data_repairs_existing_demo_dataset(memory_session: Session) -> None:
    ...
    memory_session.add_all([dataset, round_one, existing_user, existing_note])
...
def test_seed_demo_data_rolls_back_seed_writes_and_preserves_caller_work(memory_session: Session, monkeypatch: pytest.MonkeyPatch) -> None:
